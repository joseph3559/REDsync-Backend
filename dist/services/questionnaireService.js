...
export async function processUploadedFiles(files, userId) {
    ...
    for (const f of files) {
        ...
        try {
            ...
            console.error(`Failed to parse ${f.originalname}:`, e?.message || e);
            continue;
        }
    }
    return { questionnaireId: q.id, parsedQuestions: allQuestions };
}
export async function processQuestionnaire(questionnaireId, parsedQuestions, userId) {
    ...
    if (company) {
        ...
        for (const [k, v] of entries) {
            if (v && String(v).trim())
                companyInfo.push({ key: k, value: String(v).trim() });
        }
    }
    ...
    const saved = await prisma.$transaction([
        ...
        ...answersToSave.map((a) => prisma.questionnaireAnswer.create({
...
export async function saveDraft(questionnaireId, answers, userId) {
    const questionnaire = await prisma.questionnaire.findUnique({ where: { id: questionnaireId } });
    if (!questionnaire)
        throw new Error("Questionnaire not found");
...
export async function getQuestionnaires(userId, status = "all", limit = 20) {
...
export async function deleteQuestionnaire(questionnaireId, userId) {
    const questionnaire = await prisma.questionnaire.findUnique({ where: { id: questionnaireId } });
    if (!questionnaire)
        throw new Error("Questionnaire not found");
    if (questionnaire.userId !== userId)
...
function shouldSkipQuestion(lowerQuestion, certs) {
    // Basic skip logic examples; can be expanded
    if (certs.includes("fsma") && lowerQuestion.includes("fsma"))
        return true;
    if (certs.includes("haccp") && lowerQuestion.includes("haccp"))
        return true;
    if (certs.includes("fssc22000") && lowerQuestion.includes("fssc"))
        return true;
    if (certs.includes("kosher") && lowerQuestion.includes("kosher"))
        return true;
    if (certs.includes("halal") && lowerQuestion.includes("halal"))
        return true;
    return false;
}
...
async function extractQuestionsFromPdf(filePath) {
    ...
    return lines
        .filter((l) => isQuestionLine(l))
        .map((text, idx) => ({ id: `pdf-${idx}`, text }));
}
async function extractQuestionsFromDocx(filePath) {
    ...
    return lines
        .filter((l) => isQuestionLine(l))
        .map((text, idx) => ({ id: `docx-${idx}`, text }));
}
async function extractQuestionsFromXlsx(filePath) {
...
async function mergeAnswersAndSign(originalPath, answers) {
    ...
    if (signedPdf !== outPath)
        await fs.promises.copyFile(signedPdf, outPath);
    return outPath;
}
async function annotatePdfWithAnswers(pdfPath, answers) {
...
async function generateAnswersPdf(answers) {
