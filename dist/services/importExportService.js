...
const KNOWN_COMPETITOR_PRODUCTS = [
...
export async function processImportExportFiles({ files, userId }) {
    ...
    for (const file of files) {
        ...
        for (const raw of json) {
            ...
            const productCandidate = String(raw["Product"] || raw["Description"] || raw["Commodity"] || raw["Item"] || raw["Goods"] || "");
            ...
            Object.defineProperty(row, "__meta__", {
                value: {
                    ...
                    matchConfidence: fuzzy?.score != null ? 1 - fuzzy.score : undefined,
            ...
            allRows.push(row);
            logs.push(`[${file.originalname}] Product='${row["Product Name"]}' Company='${row.__meta__?.company ?? "-"}' Flow='${row.__meta__?.flow ?? "-"}' HS='${row.__meta__?.hsCode ?? "-"}'`);
            // Persist
            try {
                await prisma.importExportRecord.create({
                    data: {
                        ...
                        company: String(row.__meta__?.company || ""), // Changed from country - now stores exporter name for exports, supplier name for imports
                        flow: String(row.__meta__?.flow || "unknown"),
                        hsCode: String(row.__meta__?.hsCode || ""),
                        ...
                        competitorName: row.__meta__?.competitorName || null,
                        matchedProduct: row.__meta__?.redEquivalent || null,
                        matchConfidence: row.__meta__?.matchConfidence ?? null,
                        rawText: combinedText.slice(0, 2000),
                        metadata: row.__meta__,
                    },
                });
            }
            catch (e) {
...
function inferIncotermFromRow(row) {
    const text = Object.values(row).join(" ").toUpperCase();
    const incos = ["FOB", "CIF", "CFR", "DAP", "EXW", "DDP", "FCA", "CPT", "CIP"];
    for (const i of incos)
        if (text.includes(i))
            return i;
    return "";
}
function extractCompanyName(importerExporter, flow) {
    ...
    try {
        const resp = await axios.post("https://api.openai.com/v1/chat/completions", {
            ...
            messages: [
                {
                    role: "system",
                    content: "Extract trade entities. Return a compact JSON with keys: productName, company, price, quantity, currency, incoterm, competitor, hsCode. If unknown, use empty string or null.",
                },
                {
                    role: "user",
                    content: `Text: ${text}\nHS hint: ${hints.hs ?? ""}`,
