...
async function logAudit(userId, action, module) {
    try {
...
router.post("/company", authenticate, upload.single("logo"), async (req, res) => {
    try {
        ...
        await logAudit(req.userId, "update_company_info", "settings");
...
router.post("/users", authenticate, async (req, res) => {
    try {
        ...
        await logAudit(req.userId, "create_user", "settings");
...
router.put("/users/:id", authenticate, async (req, res) => {
    try {
        ...
        await logAudit(req.userId, "update_user", "settings");
...
router.delete("/users/:id", authenticate, async (req, res) => {
    ...
    await logAudit(req.userId, "delete_user", "settings");
...
router.post("/coa", authenticate, async (req, res) => {
    try {
        ...
        await logAudit(req.userId, "update_coa_settings", "settings");
        return res.json({ data: saved });
    }
    catch (e) {
        return res.status(400).json({ message: e.message });
...
router.post("/import-export", authenticate, async (req, res) => {
    try {
        ...
        await logAudit(req.userId, "update_import_export_settings", "settings");
        return res.json({ data: saved });
    }
    catch (e) {
        return res.status(400).json({ message: e.message });
...
router.post("/questionnaires", authenticate, upload.array("certifications", 10), async (req, res) => {
    try {
        const raw = req.body || {};
        const coerced = {
        ...
        const uploaded = req.files?.map(f => `/uploads/settings/certifications/${f.filename}`) ?? [];
        ...
        await logAudit(req.userId, "update_questionnaire_settings", "settings");
        return res.json({ data: saved });
    }
    catch (e) {
        return res.status(400).json({ message: e.message });
...
router.post("/ai", authenticate, async (req, res) => {
    try {
        ...
        const toSave = { model: body.model, confidenceThreshold: body.confidenceThreshold };
        if (body.openAiKey)
            toSave.openAiKey = encryptString(body.openAiKey);
        const saved = existing
            ? await prisma.aISettings.update({ where: { id: existing.id }, data: toSave })
            : await prisma.aISettings.create({ data: toSave });
        await logAudit(req.userId, "update_ai_settings", "settings");
...
router.post("/system", authenticate, async (req, res) => {
    try {
        ...
        await logAudit(req.userId, "update_system_preferences", "settings");
        return res.json({ data: saved });
    }
    catch (e) {
        return res.status(400).json({ message: e.message });
