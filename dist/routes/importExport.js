...
router.get("/analytics/monthly", authenticate, async (req, res) => {
    try {
        const { product, company, year } = req.query;
        const y = year ? Number(year) : undefined;
        const records = await prisma.importExportRecord.findMany({
            where: {
                AND: [
                    { userId: req.userId },
                    product ? { matchedProduct: { equals: product, mode: "insensitive" } } : {},
                    company ? { company: { contains: company, mode: "insensitive" } } : {}, // Changed to filter by company field instead of importerExporter
                ],
            },
            select: {
                flow: true,
        ...
        function ensure(month) {
            if (!monthly[month])
                monthly[month] = { import: { kg: 0, usd: 0, usdPerKg: 0 }, export: { kg: 0, usd: 0, usdPerKg: 0 } };
        ...
        for (const r of records) {
            ...
            if (!period)
                continue;
            if (y && !String(period).startsWith(String(y)))
                continue;
...
router.get("/analytics/grouped", authenticate, async (req, res) => {
    try {
        ...
        const rows = records.map((r) => {
            const period = r.metadata?.period || null;
            return {
                ...
                importerName: r.metadata?.importerName || "",
                importerCountry: r.metadata?.importerCountry || "",
            };
        });
