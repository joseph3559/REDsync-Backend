...
import { 
  ...
  getPhase1CoaColumns, 
  getPhase2CoaColumns,
  getCoaColumnsWithConfig,
  getPhase1ColumnsWithConfig,
  getCoaColumnsFromExcel
...
function mapExtractedDataToDbFields(extractedData: Record<string, unknown>) {
...
function normalizeSampleId(sampleId: string | null | undefined): string | null {
...
async function upsertCoaRecord(userId: string, fileName: string, dbData: Record<string, any>) {
  ...
  if (!sampleId || !batchId) {
    ...
    if (filenameIds.sampleId) dbData.sampleId = filenameIds.sampleId;
    if (filenameIds.batchId) dbData.batchId = filenameIds.batchId;
  }
  ...
  try {
    ...
    for (const row of rows) {
      const fields = headers.map((header) => {
        ...
        if (header === "") return "";
        return escapeCsv((row as Record<string, unknown>)[header]);
      });
...
router.post("/cleanup-duplicates", authenticate, async (req, res) => {
  try {
    ...
    for (const record of allRecords) {
      ...
      if (!normalizedSampleId || !normalizedBatchId) continue;
    ...
    for (const [groupKey, records] of Object.entries(recordGroups)) {
      ...
      const masterExtractedSampleId = (masterRecord as any).extractedSampleId;
      const masterExtractedBatchId = (masterRecord as any).extractedBatchId;
      ...
      for (const record of records) {
        for (const [key, value] of Object.entries(record)) {
          if (key === 'id' || key === 'userId' || key === 'createdAt' || key === 'extractedSampleId' || key === 'extractedBatchId') continue;
          
          if (value !== null && value !== undefined && value !== '') {
            // If master doesn't have this value, or duplicate has newer data
            if (!masterRecord[key as keyof typeof masterRecord] || record.createdAt > masterRecord.createdAt) {
...
router.post("/test-normalization", authenticate, async (req, res) => {
  try {
    ...
    const results = sampleIds.map(sampleId => ({
      ...
      matches: sampleIds.filter(otherId => 
        normalizeSampleId(otherId) === normalizeSampleId(sampleId) && otherId !== sampleId
      )
    }));
  ...
  try {
    ...
    return res.json({ 
      ...
      testCases: [
        ...
        passed: 
          extractIdsFromFilename(test.input).sampleId === test.expectedSample &&
