...
const PARAMETER_DEFINITIONS: Record<string, { fullName: string; definition: string; unit?: string }> = {
  'AI': { 
...
export function getPhase1Columns(): ColumnConfig[] {
  return HEADER_CONFIG.filter(col => col.phase === 1 && !col.ignore);
}

export function getPhase2Columns(): ColumnConfig[] {
  return HEADER_CONFIG.filter(col => col.phase === 2 && !col.ignore);
}

export function getAllActiveColumns(): ColumnConfig[] {
  return HEADER_CONFIG.filter(col => !col.ignore);
}

export function getIgnoredColumns(): ColumnConfig[] {
  return HEADER_CONFIG.filter(col => col.ignore);
}

export function getColumnsByLaboratory(lab: string): ColumnConfig[] {
  return HEADER_CONFIG.filter(col => col.laboratory.includes(lab) && !col.ignore);
}

export function getColumnsByType(type: string): ColumnConfig[] {
  return HEADER_CONFIG.filter(col => col.type === type && !col.ignore);
}

export function getColumnConfig(columnName: string): ColumnConfig | undefined {
  return HEADER_CONFIG.find(col => col.name === columnName);
}

export function getParameterDefinition(parameterCode: string): string {
